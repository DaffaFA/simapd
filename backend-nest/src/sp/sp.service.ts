...
export class SpService {
  ...
  async countViolationsForPersonnel(personnelId: string): Promise<number> {
    return this.spRepo.manager.createQueryBuilder(Violation, 'v')
      .leftJoin('v.links', 'vl')
  ...
  async generateLetter(spId: string, issuedByUsername: string): Promise<Buffer> {
      ...
      try {
        ...
        doc.text(
          `Oleh karena itu, dengan ini diterbitkan ${sp.level} (${
            sp.level === 'SP1' ? 'Surat Peringatan Pertama'
